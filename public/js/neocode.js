...
function login() {
  ...
  setTimeout(() => {
    if (username === "abcd" && password === "123456") {
      ...
      document.getElementById("mainInterface").style.display = "block";
      showNotification(
        "Welcome back, abcd! Ready to book your ride.",
        "success"
      );
      loginText.style.display = "inline";
      loginLoader.style.display = "none";
...
function updateMap(pickup, dropoff) {
  ...
  mapFrame.src = `https://www.google.com/maps/embed/v1/directions?key=YOUR_Maps_API_KEY&origin=${encodeURIComponent(
    pickup
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        window.open(url, "_blank");
        showNotification(
          "Could not get precise location, sending general emergency alert.",
          "error"
        );
      }
    );
    ...
    window.open(url, "_blank");
    showNotification(
      "Geolocation not supported, sending general emergency alert.",
      "error"
    );
  }
}
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        console.log("Current Live Location:", {
          lat,
          lng,
          link: googleMapsLink,
        });
        ...
        const url = `https://wa.me/${nikhilPhone}?text=${encodeURIComponent(
          message
        )}`;
        window.open(url, "_blank");
...
setInterval(() => {
  ...
  if (statusIndicator) {
    ...
    setTimeout(
      () => (statusIndicator.style.animation = "bounce 1s infinite"),
      10
    );
  }
}, 10000);
